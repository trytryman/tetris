...
public class TetrisCanvas extends JPanel implements Runnable, KeyListener {
			...
			try { 
				if(makeNew) { 
					...
					if(current.moveDown()){
						...
						if(current.copy()) { 
							stop();
							exp[exs]=data.now_score;
							Integer[] ex = Arrays.stream(exp).boxed().toArray(Integer[]::new);
							Arrays.sort(ex,Comparator.reverseOrder());
	...
	public void keyPressed(KeyEvent e) {
		...
		case 39:current.moveRight(); repaint(); break;
